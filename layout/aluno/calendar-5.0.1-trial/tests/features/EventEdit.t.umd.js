...
StartTest(t => {
  ...
  t.it('Should be able to change the name and date', async t => {
    ...
    t.chain({
      ...
      calendarField = eventEditor.widgetMap.resourceField; // Check that the title has been translated.
  ...
  t.it('UI should be locked when Calendar is readOnly', async t => {
    ...
    next => {
      ...
      t.ok(widgetMap.deleteButton.hidden, 'Delete button hidden'); // UI must be locked
  ...
  t.it('Drag-creating an interDay event in WeekView', async t => {
    ...
    t.chain(next => {
      ...
      waitFor: () => {
        ...
        editRecurrenceButton = (_editor2 = editor) === null || _editor2 === void 0 ? void 0 : _editor2.widgetMap.editRecurrenceButton; // Editor contains focus and is aligned to the new event's element.
  ...
  t.it('Event preserve currentTimeIndicator when doing autoCreate in current day', async t => {
    ...
    t.chain({
      ...
      indicator = els[0]; // make sure 300px offset is not scrolled off the top:
  ...
  t.it('Editing an event which is in a hidden nonworking day should edit', async t => {
    ...
    await t.waitForSelector('.b-eventeditor'); // The editor must successfully start
  ...
  t.it('Stores should report added records correctly', async t => {
    let assignmentStore, eventStore; // We need exact config without any possible side effects from the utility code
  ...
  t.it('showAllDayHeader : false', async t => {
    ...
    await t.click('.b-tool[data-ref="edit"]'); // Event editor shown is pass condition. Bug was a JS error thrown.
