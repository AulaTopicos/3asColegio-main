...
StartTest(t => {
  ...
  t.it('Event bar colours should come from resource\'s eventColor', async t => {
    ...
    await t.waitForAnimations(); // Check that event bars are the correct colour
  ...
  t.it('modeDefaults', async t => {
    ...
    calendar.mode = 'week'; // Only work days are visible

    await t.waitForAnimations();
    t.selectorCountIs('.b-weekview .b-cal-cell-header:visible', 5);
    calendar.modeDefaults.hideNonWorkingDays = false; // All days now visible

    t.selectorCountIs('.b-weekview .b-cal-cell-header:visible', 7);
    calendar.mode = 'month';
    await t.waitForAnimations(); // All days now visible
  ...
  t.it('modeDefaults not reapplying initial defaults', async t => {
    ...
    t.selectorCountIs('.b-monthview .b-calendar-day-header:visible', 5); // Update only the month view to show non working days

    calendar.activeView.hideNonWorkingDays = false;
    await t.waitFor(() => t.query('.b-monthview .b-calendar-day-header:visible').length === 7); // This should just update the minDayWidth of modes which understand.
    // It should *not* reapply initial modeDefaults to child views.

    calendar.modeDefaults.minDayWidth = 200; // All days still visible. The setting of minDayWidth in modeDefaults
    // should not have reapplied the initial default of hideNonWorkingDays : true.

    t.selectorCountIs('.b-monthview .b-calendar-day-header:visible', 7);
    calendar.mode = 'week';
    await t.waitForAnimations(); // Setting was applied to the week. Five days of width 200
  ...
  t.it('Datepicker with events', async t => {
    ...
    const c = t.query('.b-sidebar .b-calendar-cell[data-date="2019-10-15"]')[0]; // Cells with bullets are expanded
  ...
  t.it('datePicker', async t => {
    ...
    eventStore.remove(1); // Wait for next refresh of DatePicker

    await t.waitForSelector('.b-calendar-cell[data-date="2019-10-14"] .b-cell-events-badge:contains(3)'); // Badges update
  ...
  t.it('should align overflow popup correctly in a scrolled page', async t => {
    ...
    t.isApproxPx(xy[1], 238, 6); // Anchor correctly colour matched
  ...
  t.it('Drilling down', async t => {
    ...
    }, async () => t.is(calendar.mode, 'day'), next => {
    ...
    }, async () => t.is(calendar.mode, 'week'), // Move the WeekView that we are going to be drilling to to
    // a date range that would be *incorrect* if we saw it.
    {
      ...
      click: 'button:contains(Year)'
    }, async () => t.is(calendar.mode, 'year'), // The dblclick we do to drill must not trigger the overflow popup
    next => {
    ...
    // a date range that would be *incorrect* if we saw it.
    {
      ...
      click: 'button:contains(Year)'
    }, async () => t.is(calendar.mode, 'year'), // Single click on a year cell should show the overflow popup
    {
  ...
  t.it('Should fire just one refresh event upon first render', async t => {
    ...
    calendar.mode = 'agenda'; // Wait for the bug condition: extraneous refresh events arriving.
