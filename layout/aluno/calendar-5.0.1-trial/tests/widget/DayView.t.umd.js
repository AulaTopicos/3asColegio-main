...
StartTest(t => {
  ...
  function checkEventSanity(t, dayView) {
    ...
    events.forEach(event => {
      ...
      if (eventEl) {
        t.isApproxPx(eventEl.offsetTop, dayView.hourHeight * (getDateHours(event.startDate) - dayView.dayStartHour), `Event ${event.name} top correct`); // dayEndTime truncates height.
  ...
  t.it('sanity', async t => {
    ...
    await t.waitForAnimationFrame(); // This should have no effect.

    dayView.dayContainerElement.scrollTop = 100;
    t.isApproxPx(dayView.dayContainerElement.scrollTop, 0, 'Background el sizing correct, causes no scrolling'); // The day element is exactly the correct number of pixels high.

    t.isApproxPx(dayView.getDayElement(dayView.startDate).offsetHeight, dayView.hourHeight * dayView.getDayLength('hours'));
    dayView.startHour = 2; // Wait for layout to be correct

    await t.waitForAnimationFrame(); // The day element is exactly the correct number of pixels high.
  ...
  t.it('Current time line', async t => {
    ...
    t.chain( // Wait for layout to be correct
    {
      waitForAnimationFrame: null
    }, () => {
      const indicator = document.querySelector('.b-current-time-indicator'); // Check that its percentage progress through the day is correct
  ...
  t.it('Midnight to midnight events must be in the all day region', async t => {
    ...
    t.chain( // Wait for layout to be correct
    {
      ...
      const el = t.query('.b-calendarrow .b-cal-event-wrap.b-allday:contains(Midnight to midnight)')[0]; // It's 1/8 of the view width. We use an 8 day view for simple arithmetic.
  ...
  if (DomHelper.scrollBarWidth) {
    t.it('There must be no padding in header if there\'s no scrollbar', async t => {
      ...
      let lastDayContainerBackground = DomHelper.getStyleValue(dayView.dayContainerElement, 'backgroundImage'); // Loop from no scrollbar to showing a scrollbar, ensuring that at each 0.1 pixel point
  ...
  t.it('Changing event startDate should work', async t => {
    ...
    t.chain({
      ...
      trigger: () => {
        ...
        t.is(dayView.getEventElement(dinner), tuesdayCell.children[2]); // Move to start before breakfast
      ...
      waitForEvent: [dayView, 'refresh'],
      trigger: () => {
        t.selectorCountIs('.b-cal-event-wrap', 3, 'Three events are rendered'); // The dinner element is in the correct order. It must now be the first element

        t.is(dayView.getEventElement(dinner), tuesdayCell.firstElementChild); // The breakfast element is in the correct cell

        t.is(dayView.getEventElement(breakfast), tuesdayCell.children[1]); // Move to previous day

        breakfast.startDate = new Date(2019, 9, 14, 7);
      }
    }, () => {
      t.selectorCountIs('.b-cal-event-wrap', 3, 'Three events are rendered'); // The breakfast element is in the correct cell
  ...
  const checkDayLayout = async (t, month, day, minWidth, eventCount) => {
    ...
    await t.waitForSelector(`.b-calendarrow-header .b-day-name-date:contains(${day})`); // wait for the events to render:

    await t.waitForSelector('.b-cal-event-wrap');
    const eventEls = dayView.dayContentElement.querySelectorAll('.b-cal-event-wrap'),
          space = Rectangle.from(dayView.dayContainerElement); // This is harder to get right than it seems (across tz's the above loop does not always get the right number)
    // t.is(eventEls.length, eventCount, `Correct number of rendered events for the ${day}th`);
  ...
  t.it('visibleStartTime < dayStartTime', async t => {
    ...
    await t.waitForAnimationFrame(); // DayView.getPositionFromTime did not handle the case where visibleStartTime < dayStartTime
  ...
  t.it('visibleStartTime < dayStartTime', async t => {
    ...
    dayView = await getDayView(t, {
      ...
      dayStartShift: 10,
      visibleStartTime: 1
    }, false); // Wait any scroll to jhave been layed out
