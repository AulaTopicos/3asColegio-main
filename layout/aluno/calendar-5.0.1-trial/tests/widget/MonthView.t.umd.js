...
StartTest(t => {
  ...
  const iconClasses = ['b-fa-circle', 'b-fa-flag', 'b-fa-clock', 'b-fa-cloud', 'b-fa-cog', 'b-fa-diamond'],
        trimNewLines = /[\r\n]/g; // eslint-disable-next-line no-unused-vars
  ...
  async function getMonthView(config) {
    ...
    const monthView = await t.getMonthView(config); // eslint-disable-next-line no-unused-vars
  ...
  t.it('sanity', async t => {
    ...
    firstDayOfMonth.setDate(monthView.month.startDayOfMonth); // align-items : center must only come in at the DatePicker level

    t.hasStyle(monthView.element.querySelector('.b-calendar-cell'), 'alignItems', 'normal'); // Non-inclusive, or, if you prefer, it refers not to a 24 hour span, but
    // to the exact millisecond point that the view ends.

    lastDayOfMonth.setDate(monthView.month.endDayOfMonth + 1); // Work out what today's cell should contain.
    // If it's column 0, we have to include the week number.

    const todayCellText = `${today.getDay() === monthView.month.weekStartDay ? monthView.month.getWeekNumber(today)[1] : ''}${String(today.getDate())}`; // Check that the cell is accessed and contains the day's number
    ...
    t.notOk(monthView.endDate - lastDayOfMonth, 'endDate is correct'); // No Events

    t.selectorCountIs('.b-cal-event-wrap', 0); // Check that zero duration events (reminders) at limits are rendered
    ...
    t.selectorExists(`[data-date="${DateHelper.format(monthView.startDate, 'YYYY-MM-DD')}"] [data-event-id="${firstCellEvent.id}"].b-cal-event-wrap`, 1); // endDate is exclusive. It's 00:00:00 of the date after the last cell
    ...
    await t.waitForEvent(monthView, 'refresh');
    t.selectorExists(`[data-date="${DateHelper.format(DateHelper.add(monthView.endDate, -1, 'day'), 'YYYY-MM-DD')}"] [data-event-id="${lastCellEvent.id}"].b-cal-event-wrap`, 1); // Check that hideNonWorkingDays works
    ...
    t.is(Array.from(monthView.contentElement.querySelectorAll(`.${monthView.dayCellCls}`)).filter(isNotHideDisplay).length, 30); // These two comples methods should not be run by default.
  ...
  t.it('hideNonWorkingDays', async t => {
    ...
    const event = calendar.eventStore.first,
          eventEl = calendar.activeView.getEventElement(calendar.eventStore.first); // It only covers 2 of the 4 cells (8 & 10) because Sunday, Wednesday (9th) and Saturday are all missing

    t.is(parseInt(eventEl.style.width), 2 / 4 * 100, 'width correct'); // Runs from Tue 8th to Thu 10th
  ...
  t.it('One day events', async t => {
    ...
    monthView.eventElements.forEach(el => t.isApprox(el.getBoundingClientRect().width, monthView.weekWidth / 7, 0.5)); // Test fix for https://github.com/bryntum/support/issues/3417
  ...
  t.it('Two day events', async t => {
    ...
    const allDayEventRule = CSSHelper.findRule('.b-solid-bar .b-cal-event'),
          allDayBackgroundColor = allDayEventRule.style.getPropertyValue('background-color'); // Check that the event bars have correctly inherited the event's resource's declared color

    t.ok(chunk0BackgroundColor === 'rgb(0, 0, 255)' || chunk0BackgroundColor === '#0000ff');
    t.ok(chunk1BackgroundColor === 'rgb(0, 0, 255)' || chunk1BackgroundColor === '#0000ff'); // Should drop to using the default all day event background colour

    e5.resource.eventColor = null;
    await t.waitForEvent(monthView, 'refresh'); // Reacquire colour styles now that they must have changed.

    chunk0BackgroundColor = window.getComputedStyle(e5El[0].querySelector('.b-cal-event')).backgroundColor;
    chunk1BackgroundColor = window.getComputedStyle(e5El[1].querySelector('.b-cal-event')).backgroundColor; // Check that the event bars have correctly dropped back to the default background colour
  ...
  t.it('Multi day spanning events', async t => {
    ...
    let cell7 = monthView.cellMap.get('2011-01-07'),
        ...
        cell14new1; // Two events covering the 8th
    // 4, and the 8th's own event, 5

    t.is(cell8Events.slots.length, 2); // 4 was at slot 1 in its originating cell, so that's where it is in this cell.
    // So 5 went in at slot 0

    t.isDeeply(renderedIds, [5, 4]); // No overflow indicators

    t.selectorCountIs('.b-cal-cell-overflow', 0);
    e1.duration = 5;
    await t.waitForEvent(monthView, 'refresh'); // Wait for the data state to be correct

    await t.waitFor(() => {
      cell8Events = monthView.cellMap.get('2011-01-08').renderedEvents;
      return cell8Events.slots.length === 3;
    });
    renderedIds = cell8Events.slots.map(s => s.eventRecord.id); // Now there are three events covering the 8th.
    // 1, 4, and the 8th's own event, 5

    t.is(cell8Events.slots.length, 3);
    t.isDeeply(renderedIds, [1, 4, 5]); // Still no overflow indicators
    ...
    t.isDeeply(renderedIds, [1, 2, 5, 4]); // Still no overflow indicators
    ...
    cell8 = monthView.cellMap.get('2011-01-08');
    cell8Events = cell8.renderedEvents;
    renderedIds = cell8Events.slots.map(s => s.eventRecord.id); // Both 7th and 8th must be flagged as overflowing.
    ...
    t.is(cell8.hasOverflow, true); // Now there are three events covering the 8th.
    // 1, 2, the 8th's own event, 5, then 4, then the new, intra-day event

    t.is(cell8Events.slots.length, 5);
    t.isDeeply(renderedIds, [1, 2, 5, 4, cell8new1.id]); // Start interacting
    ...
    t.chain(next => {
      ...
      e1.duration = 11;
      await t.waitForEvent(monthView, 'refresh'); // e1 has now nudged over into next week
      ...
      e2.duration = 10;
      await t.waitForEvent(monthView, 'refresh'); // e2 now extends to the 14th
      ...
      e3.duration = 9;
      await t.waitForEvent(monthView, 'refresh');
      t.selectorExists('[data-date="2011-01-08"] .b-cal-cell-overflow:contains(+3 more)'); // e3 now extends to the 14th
      ...
      e5.duration = 7;
      await t.waitForEvent(monthView, 'refresh'); // e5 now extends to the 14th
      ...
      t.is(monthView.cellMap.get('2011-01-14').hasOverflow, true, '14th has overflow'); // The 14th has a new one
  ...
  t.it('No events intersecting the month', async t => {
    ...
    const eventElements = monthView.bodyElement.querySelectorAll('.b-cal-event-wrap:not(.b-overflow)');
    t.is(eventElements.length, 0, 'No event elements'); // No event data cells
  ...
  t.it('Events intersecting start and end of month by one day', async t => {
    ...
    t.selectorCountIs('.b-cal-event-wrap.b-allday.b-continues-right', 1); // Two event data cells
  ...
  t.it('No events starting or ending in the month, but an event *spanning* the month', async t => {
    ...
    const eventElements = monthView.bodyElement.querySelectorAll('.b-cal-event-wrap:not(.b-overflow)');
    t.is(eventElements.length, 6, 'six event start elements'); // We should only have gathered cells for the view's own cells
  ...
  t.it('Events overflowing a cell', async t => {
    ...
    t.firesOnce(monthView, 'cellOverflowClick', 'Overflow indicator clicked only once'); // At height 500, there's only space for two events
    ...
    t.chain({
      ...
      t.selectorCountIs('.b-cal-event-wrap', 2); // Test fix for https://github.com/bryntum/support/issues/3409
      ...
      t.selectorCountIs(`#${monthView.overflowPopup.id} .b-cal-event-wrap`, 4); // Test fix for https://github.com/bryntum/support/issues/3409
    ...
    }, // When were down to three, there's no need for the overflow indicator
    {
      ...
      t.selectorCountIs('.b-cal-event-wrap:visible', 3); // Test fix for https://github.com/bryntum/support/issues/3409
    ...
    monthView = await getMonthView({
      ...
      dayCellRenderer: function (cellData) {
        if (!(cellData.date - new Date(2011, 0, 3))) {
          ...
          cellData.headerStyle.textDecoration = 'underline'; // Mutate day cell information
  ...
  t.it('Events at visible limits', async t => {
    ...
    t.is(monthView.cellMap.size, 2); // other month indicator shows whether it's past or future
  ...
  t.it('dayCellRenderer should be able to affect cell style', async t => {
    ...
    const mEl = monthView.element; // First refresh should have applied cell styles, different for each day of the week
    ...
    monthView.doRefresh(); // Conditions still exactly the same
    ...
    setColour = false;
    monthView.refresh(); // Style must be cleared now
  ...
  t.it('interday events should sort to the top', async t => {
    ...
    t.selectorCountIs('.b-cal-event-wrap:not(.b-allday)', 0); // The 10th has three overflows

    t.selectorCountIs('.b-cal-cell-overflow:contains(+3 more)', 1); // The 11th does not itself overflow, but because the previous cell has an overflow
    ...
    const eventEls = monthView.overflowPopup.element.querySelectorAll('.b-cal-event-wrap'); // Order must be correct. Interday events all gone to top.
  ...
  t.it('should show the correct number of overflow events even when extremely small height', async t => {
    ...
    t.selectorCountIs('.b-cal-cell-overflow:contains(+5 more)', 1); // The 11th has 3 overflows
  ...
  t.it('Week row sizing', async t => {
    ...
    await t.click('.b-cal-cell-overflow'); // Row animates to new height.
    // Sets the Widget isAnimating state which Siesta knows how to wait for.

    await t.waitForAnimations(); // weekShrinkwrap event was fired correctly

    t.is(shrinkWrappedElement, weekRow, 'Event recieved correct element');
    t.is(shrinkWrappedWeekStart, weekStartDate, 'Event recieved correct date');
    t.is(parseInt(DomHelper.getStyleValue(weekRow, 'flex-grow')), 0, 'Test week row flex-grow is 0');
    t.is(parseInt(DomHelper.getStyleValue(weekRow, 'flex-shrink')), 0, 'Test week row flex-shrink is 0');
    t.isApprox(parseInt(DomHelper.getStyleValue(weekRow, 'flex-basis')), 128, 1, 'Test week row flex-basis is 128'); // Week has expanded

    t.isApprox(weekRow.offsetHeight, 128, 'Expanded height correct');
    t.isGreater(weekRow.offsetHeight, flexedHeight, 'Expanded height is greater than flexed height'); // It's scrollable in the Y axis

    t.ok(monthView.scrollable.maxY, 'MonthView has vertical scroll'); // There's no overflow indicator

    t.selectorNotExists('.b-cal-cell-overflow', 'No visible overflow indicator'); // Set it back to a flexed height

    monthView.flexWeekRow(2); // Row animates to new height.
    // Sets the Widget isAnimating state which Siesta knows how to wait for.

    await t.waitForAnimations(); // weekFlex event was fired correctly

    t.is(flexedElement, weekRow, 'Event recieved correct element');
    t.is(flexedWeekStart, weekStartDate, 'Event recieved correct date'); // Week has shrunk back

    t.is(parseInt(DomHelper.getStyleValue(weekRow, 'flex-grow')), 1, 'Test week row flex-grow is 1');
    t.is(parseInt(DomHelper.getStyleValue(weekRow, 'flex-shrink')), 0, 'Test week row flex-shrink is 0');
    t.is(parseInt(DomHelper.getStyleValue(weekRow, 'flex-basis')), 0, 'Test week row flex-basis is 0');
    t.is(weekRow.offsetHeight, flexedHeight, 'Collapsed height correct'); // It's not scrollable in the Y axis

    t.notOk(monthView.scrollable.maxY, 'MonthView has vertical scroll'); // There's an overflow indicator
  ...
  t.it('eventHeight', async t => {
    ...
    const e1Element = monthView.getEventElement(eventStore.first).querySelector('.b-cal-event'),
          ...
          defaultHeight = DomHelper.setLength(monthView.eventHeight); // Event elements are obeying the eventHeight, and not overlapping each other

    t.is(e1Element.style.height, defaultHeight);
    t.is(e2Element.style.height, defaultHeight);
    t.notOk(Rectangle.from(e1Element).intersect(Rectangle.from(e2Element))); // Make event heights use a string based height using CSS units

    monthView.eventHeight = '3em'; // Await the *next* refresh event
  ...
  t.it('Should switch to month view upon click of a year\'s month name header', async t => {
    ...
    t.selectorNotExists('.b-cal-event-wrap.b-allday.b-past-event.b-continues-right:contains(Hackathon 2019)', 'Month view not shown');
    ...
    await eventWait; // Now we must be seeing the hackathon month

    t.selectorExists('.b-cal-event-wrap.b-allday.b-past-event.b-continues-right:contains(Hackathon 2019)', 'Month view shown');
  ...
  t.it('overflowing events bug 2', async t => {
    ...
    const m = calendar.activeView; // Events which intersect a date occupy different cells.
  ...
  t.it('reconfiguring overflowPopup', async t => {
    ...
    t.selectorNotExists('.b-overflowpopup [data-ref="eventList"]'); // Check property
